		...
		<InstantSearch searchClient={searchClient} indexName="officia_help">
			<div className="flex flex-col items-center justify-center w-full relative">
				<SearchBox
					...
					submitIconComponent={() => (
						<Search width={16} height={16} color={'var(--text-100)'} />
					)}
				...
				<NoResultsBoundary fallback={<NoResults />}>
					<EmptyQueryBoundary fallback={null}>
						<Hits
							classNames={{
								root: 'absolute top-16 left-0 flex flex-col z-10 gap-y-2 w-full bg-gray-200 rounded-md p-4'
							}}
							hitComponent={Hit}
		...
		>
			<article className="flex flex-row w-full justify-between items-center px-5 py-4 rounded-lg cursor-pointer bg-transparent hover:bg-gray-300 transition-colors gap-x-2">
				<div className="flex flex-col items-start justify-start gap-y-1 text-left">
					...
					<div>
						<h3 className="font-bold text-left text-white text-lg">
...
function NoResults() {
	...
	return (
		<div className="absolute top-16 left-0 flex flex-col z-10 gap-y-2 w-full bg-gray-200 rounded-md p-4 h-fit">
			<p className="text-white text-center flex-1  overflow-hidden text-ellipsis">
